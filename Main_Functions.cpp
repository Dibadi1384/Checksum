...
void Disk::STORE(int ID, char str[]){  
    ...
    if(hash_type==0){
        ...
        //insert if table not full and key is not already there
        else if(slot<num && alrExists==false){
            //store file in the free slot in table using index found by double hashing
            hashtable[slot]=new FileBlock(ID, slot);
    ...
    //chaining
        ...
        //store file in the free slot in table using index found by double hashing
        if(alrExists==false){
            chaintable[h].insert(chaintable[h].begin() + chainindex, new FileBlock(ID, h));
            ...
            chaintable[h][chainindex]->stored=true;
            std::cout<<"success"<<std::endl;
        }
...
int Disk::doublehash(int ID){
    ...
    while(h<num && hashtable[h]!=NULL){
        ...
        if(hashtable[h]->fileID==ID && hashtable[h]->stored==true){
            alrExists=true;
            return h;
        }    
...
int Disk::chaining(int ID){
    ...
    //instert other files
        ...
        if(i < chaintable[h].size() && chaintable[h][i]->fileID && chaintable[h][i]->stored==true){
            alrExists=true;
